...
class ProtoFaceDet(ProtocolClass):

    def __init__(self, dict_http, dict_grpc):
        super(ProtoFaceDet, self).__init__()
        # setup http
    ...
    def http_send_request(self, img_np_array):
        ...
        if json_response.status_code == 200:
            ...
            y_bboxes_output, y_cls_output = y_pred[0]["model_2"], y_pred[0]["model_2_1"]
            return np.asarray(y_bboxes_output)[np.newaxis, :], np.asarray(y_cls_output)[np.newaxis, :]
        else:
            return None
